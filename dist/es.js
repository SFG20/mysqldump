...
function getTables(connection, dbName, restrictedTables, restrictedTablesIsBlacklist) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        columns.forEach((cols, i) => {
            ...
            tables[i].columnsToDump = tables[i].columnsOrdered.filter(c => tables[i].columns[c].dump);
        });
        return tables;
    });
}
            ...
            while (tables.length > 0) {
                ...
                yield new Promise((resolve, reject) => {
                    ...
                    const where = options.where[table.name]
                        ...
                        : '';
                    const query = connection.query(`SELECT \`${table.columnsToDump.join('`,`')}\` FROM \`${table.name}\`${where}`);
                    let rowQueue = [];
